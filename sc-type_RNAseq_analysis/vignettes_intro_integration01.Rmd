...
# Insteas, this is how to install SeuratData of ifnb
...
ifnb
```
Here, first we would like to make sure where the data is.
Seurat Command cheat sheet
data access
```{r}
library(tidyverse)
colnames(ifnb) %>% length
Cells(ifnb) %>% length
Features(ifnb) %>% length
rownames(ifnb) %>% length
...
Features(ifnb[["RNA"]]) %>% length

#Expression data
#ifnb[["RNA"]]$counts.CTRL
```
...
```{r}
LayerData(object=ifnb, assay="RNA", layer="counts.CTRL")
LayerData(object=ifnb, assay="RNA", layer="data.CTRL")
...
```{r}
# handling dgCMatrix
LayerData(object=ifnb, assay="RNA", layer="counts.CTRL")@Dim
LayerData(object=ifnb, assay="RNA", layer="data.CTRL")@Dim
LayerData(object=ifnb, assay="RNA", layer="counts.CTRL")@i
LayerData(object=ifnb, assay="RNA", layer="data.CTRL")@i
```

```{r}
LayerData(object=ifnb, assay="RNA", layer="counts.CTRL")@x
LayerData(object=ifnb, assay="RNA", layer="data.CTRL")@x
```
...
```{r}
LayerData(object=ifnb, assay="RNA", layer="counts.CTRL")  %>% as.data.frame %>% select(1)
```
```{r}
col_sum_1 <- LayerData(object=ifnb, assay="RNA", layer="counts.CTRL")  %>% as.data.frame %>% select(1) %>% colSums(1)
col_sum_1
```
```{r}
log1p((LayerData(ifnb,assay="RNA", "counts.CTRL") %>% as.data.frame %>% select(1) /col_sum_1) * 10000)
```
```{r}
LayerData(ifnb,assay="RNA", "data.CTRL") %>% as.data.frame %>% select(1)
```
...
ifnb <- FindVariableFeatures(object=ifnb)
ifnb <- ScaleData(object=ifnb)
...
DimPlot(ifnb, reduction = 'umap.unintegrated', group.by=c('stim','seurat_clusters', 'seurat_annotations'))
```
...
```{r}
ifnb@meta.data %>% select(stim)
ifnb@meta.data %>% select(seurat_clusters)
```
```{r}
ifnb@reductions$umap.unintegrated
ifnb@reductions$umap.unintegrated[[1:8, 1:5]]
```
