...
```{r}
D000.data %>% filter(str_detect(row.names(.), "^MT-")) %>% dplyr::select(ends_with("AAAGCAGTTATT")) %>% sum
b<- D000.data %>% dplyr::select(ends_with("AAAGCAGTTATT")) %>% sum
# a / (b)
# b

a <- D000.data %>% filter(str_detect(row.names(.), "^MT-")) %>% dplyr::select(ends_with("AAGAAAGTGCCA")) %>% sum
b <- D000.data %>% dplyr::select(ends_with("AAGAAAGTGCCA")) %>% sum
a / (b-a)
D000.data %>% filter(str_detect(row.names(.), "^MT-")) %>% dplyr::select(ends_with("AAGGCCGAGTCA")) %>% sum / D000.data %>% dplyr::select(ends_with("AAGGCCGAGTCA")) %>% sum
D000.data %>% filter(str_detect(row.names(.), "^MT-")) %>% dplyr::select(ends_with("AAGTAAAGGGTN")) %>% sum / D000.data %>% dplyr::select(ends_with("AAGTAAAGGGTN")) %>% sum
D000.data %>% filter(str_detect(row.names(.), "^MT-")) %>% dplyr::select(ends_with("AGACTTTGAGTN")) %>% sum / D000.data %>% dplyr::select(ends_with("AGACTTTGAGTN")) %>% sum
```
```{r}
D000[["nFeature_RNA"]]
```
...
```{r}
D000@meta.data
D000.data %>% dplyr::select(ends_with("AAAGCAGTTATT")) %>% sum
D000.data %>% dplyr::select(ends_with("AAAGCAGTTATT")) %>% filter(AML371.D0_AAAGCAGTTATT!=0) %>% unlist %>% count
```
...
DimPlot(D000, reduction="pca")
```
確かに、pC1は‐10～30で分散しており、PC2はそこまで分散していない。

PC1方向で正負トップランクの遺伝子について、ランダムに選ばれたそれぞれ500細胞での値を可視化

```{r}
DimHeatmap(D000, dims=1, cells=756, balanced=TRUE)
```
```{r, fig.height=30}
DimHeatmap(D000, dims=1:15, cells=756, balanced=TRUE)
```
```{r, fig.height=30}
D034 <- RunPCA(D034, features = head(VariableFeatures(object=D034), 100000))
DimHeatmap(D034, dims=1:15, cells=756, balanced=TRUE)
```
...
```
```{r}
AML371_all.integrated %>% names
AML371_all.integrated$mal
```
