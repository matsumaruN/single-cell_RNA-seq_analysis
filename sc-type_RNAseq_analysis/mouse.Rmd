...
raw_WT <- CreateSeuratObject(counts = raw_WT.data, project = "ScRNA-seq of AD mouse", min.cells = 3, min.features = 200)
raw_WT$type <- "WT"
raw_AD <- CreateSeuratObject(counts = raw_AD.data, project = "ScRNA-seq of AD mouse", min.cells = 3, min.features = 200)
raw_AD$type<- "AD"
```
...
#Differentially expressed gene (DEG)の検???
```{r}
...
#meta.dataのseurat_clustersんも???報とes.maxの???報を統???
...
#細胞???を特定する新たなmeta.dataである"customclassif"を追???する
...
###merge.dataをWTとADで???割してlist化す???###------------------------------------
type_data <- SplitObject(merge.data, split.by = "type")
#tmpとはtemporaryのこと、意味のな???一時的なファイル
#table()を使???と???イン???ント???回数???"Freq"として保存???
#Var1,Var2列も自動的で生???される。それぞ???"ident"と"type"が???納されて??????
